...
- **Transactions as source of truth:** `Holding.quantity` and `averagePrice` are cached but derived from `Transaction` records via `recalculateFromTransactions()`. Always create or delete a Transaction then call recalculate — never write quantity/averagePrice directly. The delete-from-history flow (`HoldingDetailViewModel.confirmDeleteTransaction` / `deleteTransactionImmediately`) follows the same rule: prune the row, then recalculate. `recalculateFromTransactions()` clamps quantity at 0 if a deletion would leave a later sell underwater.
...
- **Add-ticker flow (single entry point):** A `+` toolbar button on each `AssetClassHoldingsView` opens `AddTickerSheet`, scoped to the screen's class. The screen's `assetClass` is passed straight to `BackendService.searchStocks(query:assetClass:)` so the backend can route to the right provider — `crypto` → CoinGecko, `rendaFixa` → empty (custom-only), everything else → yfinance + Brapi. This is index routing, not result filtering: the final class assigned to a chosen result still comes from `AssetClassType.detect(from:apiType:)`, so the sheet never lies about where a ticker ends up. A toolbar `+` (top-right of `AddTickerSheet`) opens `AddAssetDetailSheet(customDraft: assetClass)` — the same blank, editable-name draft Emergency Reserve uses; the search text is not carried over. Emergency Reserve skips search entirely and opens that same `customDraft` sheet directly. The portfolio root has no add button (Rename + Import only). New holdings persist via `AddAssetViewModel.addAsset(...)`.
