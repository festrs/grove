...
struct NewTransactionView: View {
    ...
    private func handleBuy(quantity: Decimal, price: Decimal) {
        ...
        if let existing = selectedHolding {
            let oldTotal = existing.quantity * existing.averagePrice
            let newTotal = quantity * price
            let combinedQty = existing.quantity + quantity
            existing.averagePrice = combinedQty > 0 ? (oldTotal + newTotal) / combinedQty : price
