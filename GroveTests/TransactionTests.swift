...
struct TransactionTests {
    ...
    @Test func congelarMigratestoQuarentena() {
        let holding = Holding(ticker: "TEST", assetClass: .acoesBR)
        holding.statusRaw = "congelar"

        #expect(holding.status == .quarentena, "Old congelar data should map to quarentena")
    }

    @Test func unknownStatusDefaultsToAportar() {
        let holding = Holding(ticker: "TEST", assetClass: .acoesBR)
        holding.statusRaw = "invalid_status"

        #expect(holding.status == .aportar, "Unknown status should default to aportar")
    }
}
